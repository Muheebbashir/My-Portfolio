...
export default function Skills() {
  ...
  useEffect(()=>{
    ...
    const tick =(now)=>{
      ...
      last=now;
      let next=x.get() + dir * SPEED * dir * dt;
  ...
  return (
    <section id="skills" ref={sectionRef} className="h-1/2 w-full pb-8 flex flex-col items-center relative bg-black text-white overflow-hidden">
      ...
      <div className="relative w-full overflow-hidden">
        ...
        >
          {repeated.map((s,i)=>(
            <div key={i} className="flex flex-col items-center gap-2 min-w-30 " aria-label={s.name} title={s.name}>
